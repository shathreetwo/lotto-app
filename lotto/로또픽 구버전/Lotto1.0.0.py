...
class LottoScreen(Screen):
    def __init__(self, **kwargs):
        ...
        layout = BoxLayout(
            orientation='vertical',
            padding=[dp(20), dp(0), dp(20), dp(0)],  # 상단 여백 제거
            spacing=dp(5)
        )
        ...
        self.label_box = BoxLayout(
            ...
            spacing=dp(10),
            size_hint=(1, 0.15),  # ↑ 이전보다 약간 높게 설정
            padding=[0, dp(10), 0, 0],  # 위쪽 여백 추가
            height=dp(70)
        )
        self.labels = []
        for _ in range(6):
            lbl = Label(
                ...
                font_name='Font',
                color=(0, 0, 0, 1),
                size_hint=(1/6, None),  # 가로 너비 비율만 지정
                height=dp(60)           # 기본값
            )

            def sync_height(instance, value):
                instance.height = instance.width  # 가로와 세로를 같게 설정
        ...
        # 📌 텍스트가 상단에 정렬되고, 박스 크기에 맞춰 자동 줄바꿈되도록
